...
function platform(overrides: Partial<PlatformApi> = {}): PlatformApi {
  return {
    ...
    agentRouting: async () => ({ defaultMode: 'mention_only' as const, overrides: [] }),
    setAgentRouting: vi.fn(async (_id, input) => ({ defaultMode: input.mode === 'inherit' ? 'mention_only' as const : input.mode, overrides: [] })),
    channels: async () => [],

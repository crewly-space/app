...
/** A failure's HTTP status, when the SDK carried one. */
...
/**
 * A reason worth showing. A bare 404 used to reach the screen as "That item
 * no longer exists.", which named nothing: here it means this server has no
 * Crewly connection to describe, or the link on the Crewly side is gone.
 */
...
export function CrewlyPanel({ api, serverName }: { api: ServicesApi; serverName: string }) {
  ...
  const load = useCallback(async () => {
    setLoadFailure('');
    try {
      setConnection(await api.crewly());
    } catch (reason) {
      setConnection(null);
  ...
  /**
   * Runs an action; when it fails because the thing it acted on is gone,
   * the fresh state is read back instead of leaving a raw not-found on screen.
   */
  const act = (work: () => Promise<CrewlyConnection | void>) => run(async () => {
    try {
      const next = await work();
      setConnection(next ?? await api.crewly());
    } catch (reason) {
